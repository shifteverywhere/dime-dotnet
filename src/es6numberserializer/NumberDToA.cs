...
/// </summary>
internal static class NumberDToA {

    public const int
        DtostrStandard = 0,              /* Either fixed or exponential format; round-trip */
        DtostrStandardExponential = 1,  /* Always exponential format; round-trip */
        DtostrFixed = 2,                 /* Round to <precision> digits after the decimal point; exponential if number is large */
        DtostrExponential = 3,           /* Always exponential format; <precision> significant digits */
        DtostrPrecision = 4;             /* Either fixed or exponential format; <precision> significant digits */


    private const int FracMask = 0xfffff;
    private const int ExpShift = 20;
    private const int ExpMsk1 = 0x100000;

    private const long FracMaskL = 0xfffffffffffffL;
    private const int ExpShiftL = 52;
    private const long ExpMsk1L = 0x10000000000000L;

    private const int Bias = 1023;
    private const int P = 53;

    private const int ExpShift1 = 20;
    private const int ExpMask = 0x7ff00000;
    private const int ExpMaskShifted = 0x7ff;
    private const int BndryMask = 0xfffff;
    private const int Log2P = 1;

    private const int SignBit = -0x80000000;
    private const int Exp11 = 0x3ff00000;
    private const int TenPmax = 22;
    private const int QuickMax = 14;
    private const int Bletch = 0x10;
    private const int FracMask1 = 0xfffff;
    private const int IntMax = 14;
    private const int NBigtens = 5;


    private static readonly double[] Tens = 
    {
    ...
    private static readonly double[] Bigtens = { 1e16, 1e32, 1e64, 1e128, 1e256 };
    ...
    private static int Hi0Bits(int x)
    {
        ...
        if ((x & 0x80000000) != 0) return k;
        k++;
        return (x & 0x40000000) == 0 ? 32 : k;
    }
    ...
    private static BigInteger D2B(double d, int[] e, int[] bits)
    {
        ...
        byte[] reverse = new byte[dblBits.Length];
        int q = dblBits.Length;
        foreach (byte b in dblBits)
        {
            reverse[--q] = b;
        }
    ...
    {
        var dBits = BitConverter.DoubleToInt64Bits(d);
        return (int)(dBits >> 32);
    }
    ...
    private static int JS_dtoa(double d, int mode, bool biasUp, int ndigits,
        IList<bool> sign, StringBuilder buf)
    {
        ...
        */
        ...
        {
            /* Infinity or NaN */
            buf.Append(((Word1(d) == 0) && ((Word0(d) & FracMask) == 0)) ? "Infinity" : "NaN");
            return 9999;
        }
        if (d == 0)
        {
        ...
        b = D2B(d, be, bbits);
        if ((i = ((int)(((uint)Word0(d)) >> ExpShift1) & (ExpMask >> ExpShift1))) != 0)
        {
            d2 = SetWord0(d, (Word0(d) & FracMask1) | Exp11);
        ...
        else
        {
            ...
            d2 = SetWord0(x, Word0(x) - 31 * ExpMsk1);
            i -= (Bias + (P - 1) - 1) + 1;
            denorm = true;
        }
        ...
        if (ilim >= 0 && ilim <= QuickMax && try_quick) {
            ...
            eps = ieps * d + 7.0;
            eps = SetWord0(eps, Word0(eps) - (P - 1) * ExpMsk1);
            if (ilim == 0)
            {
// Java                    S = mhi = null;
                d -= 5.0;
        ...
        if (mode < 2)
        {
            if ((Word1(d) == 0) && ((Word0(d) & BndryMask) == 0)
                                && ((Word0(d) & (ExpMask & ExpMask << 1)) != 0)
               )
            {
    ...
    public static void JS_dtostr(StringBuilder buffer, int mode, int precision, double d)
    {
        ...
        if (mode == DtostrFixed && (d >= 1e21 || d <= -1e21))
            mode = DtostrStandard; /* Change mode here rather than below because the buffer may not be large enough to hold a large integer. */

        decPt = JS_dtoa(d, dtoaModes[mode], mode >= DtostrFixed, precision, sign, buffer);
        ...
        if (decPt != 9999)
        {
            ...
            switch (mode)
            {
                ...
                case DtostrExponential:
                    //                    JS_ASSERT(precision > 0);
                    minNDigits = precision;
                    /* Fall through */
                    goto case DtostrStandardExponential;
                case DtostrStandardExponential:
                    exponentialNotation = true;
                    break;

                case DtostrPrecision:
                    //                    JS_ASSERT(precision > 0);
                    minNDigits = precision;
        ...
        if (sign[0] &&
            !(Word0(d) == SignBit && Word1(d) == 0) &&
            !((Word0(d) & ExpMask) == ExpMask &&
              ((Word1(d) != 0) || ((Word0(d) & FracMask) != 0))))
        {
            buffer.Insert(0, '-');
        }
    }
